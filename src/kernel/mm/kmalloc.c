...
static struct buddy_metadata * buddy_nodes;
...
void kmalloc_init(uintptr_t base_address)
{
	...
	if(BUDDY_TREE_SIZE >= PAGE_SIZE - 1) {
		buddy_size_paged = ALIGN_PAGE(BUDDY_TREE_SIZE + 1);
	} else {
	...
	//debug_info("Kmalloc Base: 0x%x\tTop: 0x%x\n", heap_base_address, heap_top_address);
}
...
void *kcalloc(size_t nmemb, size_t size)
{
	...
	if(nmemb != combined_size / size) {
		PANIC("Integer overflow on call to kcalloc!");
	}
